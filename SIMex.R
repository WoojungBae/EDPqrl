...

{
  ...
  # ------------------------------------------------------------------------------
  # ------------------------------------------------------------------------------
  DPMM_results = SURVIVAL_DPMM(temp_data, nu = nu_vec, rho = rho_vec, M = M, 
                               gibbs_iter = gibbs_iter, gibbs_burnin = gibbs_burnin, gibbs_thin = gibbs_thin, 
                               esttype = esttype, saveall = saveall)
  EDPMM_results = SURVIVAL_EDPMM(temp_data, nu = nu_vec, rho = rho_vec, M = M, 
                                 gibbs_iter = gibbs_iter, gibbs_burnin = gibbs_burnin, gibbs_thin = gibbs_thin, 
                                 esttype = esttype, saveall = saveall)
  ...
  file_true  = paste0("Results/Truth_Scn", Scn, "_N", N, "ex.txt")
  file_edpmm = paste0("Results/EDPMM_Scn", Scn, "_N", N, "ex.txt")
  file_dpmm  = paste0("Results/DPMM_Scn", Scn, "_N", N, "ex.txt")
  if (run_ID == 1) {
