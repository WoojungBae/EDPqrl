...
SURVIVAL_DPMM_MCMC = function(object,
  ...
  # a_sig2 = 3
  # b_sig2 = sig2_ref * (a_sig2 - 2) / a_sig2
  a_sig2 = 4
  b_sig2 = 0.1
...
SURVIVAL_DPMM_POST = function(object, M, nu, rho, 
  ...
  logSumExp = function(vals) {
    ...
    max_val + log(sum(exp(vals - max_val), na.rm = TRUE))
  }
  
  normalize_log_probs = function(log_prob_matrix) {
    max_log_probs = apply(log_prob_matrix, 1, max, na.rm = TRUE)
    log_probs_shifted = log_prob_matrix - max_log_probs
    probs_shifted = exp(log_probs_shifted)
    row_sums = rowSums(probs_shifted, na.rm = TRUE)
    
    zero_rows = which(row_sums == 0 | !is.finite(row_sums))
  ...
  if (!is.null(condX) && !is.null(p_condX)) {
    if (length(condX) != length(p_condX)) {
      stop("condX and p_condX must have the same length.")
    }
    
    condX <- as.numeric(condX)
    ...
    if (p_X2 > 0) {
      ...
      if (any(is_cont)) {
        ...
        condX[is_cont] <- (condX[is_cont] - con_center[q_idx]) / con_scale[q_idx]
      }
    }
  }
  ...
  # Storage
  # -----------------------------------------------------------------------------
  iter_check = max(1L, floor(n_MCMC / 10))
  ...
  for (post_reps in 1:n_MCMC) {
    ...
    # ---------------------------------------------------------------------------
    # Extract current posterior draw
    # ---------------------------------------------------------------------------
    ...
    eta_new = rbeta(p_Z, a_eta, b_eta)
    
    betaPars_mc = cbind(betaPars, beta_new)
    sig2Pars_mc = c(as.vector(sig2Pars), sig2_new)
    ...
    if (p_X1 > 0) {
      ...
      piPars_mc = matrix(numeric(0), nrow = 0, ncol = K + 1L)
    }
    
    if (p_X2 > 0) {
      tau2_new = as.numeric(rscainvchisq_cpp(p_X2, a_tau2, b_tau2))
      mu_new = rnorm(p_X2, mean = a_mu, sd = sqrt(tau2_new / b_mu))
      muPars_mc = cbind(muPars, mu_new)
      tau2Pars_mc = cbind(tau2Pars, tau2_new)
    } else {
      muPars_mc = matrix(numeric(0), nrow = 0, ncol = K + 1L)
      tau2Pars_mc = matrix(numeric(0), nrow = 0, ncol = K + 1L)
    }
    
    # ---------------------------------------------------------------------------
    # Calculate lambda_x_mc
    # ---------------------------------------------------------------------------
    if (!is.null(condX) && !is.null(p_condX)) {
      log_lambda_x_mc = rep(-Inf, K + 1L)
      ...
      if (p_X2 > 0) {
        ...
        log_margin_part1 =
          lgamma(a_tau_new) - lgamma(a_tau_half) +
      ...
      for (i in seq_along(p_condX)) {
        ...
        if (idx <= p_X1) {
          log_base_condX = log_base_condX +
            lbeta(a_pi + val, b_pi + 1 - val) - lbeta(a_pi, b_pi)
        } else {
          q = idx - p_X1
          log_base_condX = log_base_condX +
            log_margin_part1 -
            a_tau_new * log(ab_tau + b_mu_ratio * (val - a_mu)^2)
        }
      }
      ...
      for (k in 1:K) {
        ...
        for (i in seq_along(p_condX)) {
          ...
          if (idx <= p_X1) {
            ll = ll + dbinom(val, 1, piPars[idx, k], log = TRUE)
          } else {
            q = idx - p_X1
    ...
    K_mc = max(max(unique(S_x_mc)), K)
    
    n_k_mc = tabulate(S_x_mc, nbins = K + 1L)
    
    # ---------------------------------------------------------------------------
    # Draw confounders
    # ---------------------------------------------------------------------------
    X_mc = matrix(NA_real_, nrow = M, ncol = p_X1 + p_X2)
      ...
      if (p_X1 > 0) {
        for (q in 1:p_X1) {
          if (is.null(p_condX) || !(q %in% p_condX)) {
            X_mc[ind_S_x_mc, q] = rbinom(n_k_temp, 1, piPars_mc[q, k])
          }
        }
      }
    ...
    logf0_x_mc <- numeric(M)
    
    if (p_X1 > 0) {
      x_mc <- X_mc[, 1:p_X1, drop = FALSE]
      logf0_x_mc <- logf0_x_mc + rowSums(
        lbeta(a_pi + x_mc, b_pi + 1 - x_mc)
      )
    }
    
    if (p_X2 > 0) {
      x_mc <- X_mc[, (p_X1 + 1):(p_X1 + p_X2), drop = FALSE]
      logf0_x_mc <- logf0_x_mc + rowSums(
        margin_part1_mc - a_tau_new * log(ab_tau + b_mu_ratio * (x_mc - a_mu)^2)
      )
    }
    
    logf0_z_mc = log(0.5)
    logf0_xz0_mc = logf0_x_mc + logf0_z_mc
    logf0_xz1_mc = logf0_x_mc + logf0_z_mc
    ...
    lambda_Y_z0x_mc = matrix(-Inf, nrow = M, ncol = K_aug)
    lambda_Y_z1x_mc = matrix(-Inf, nrow = M, ncol = K_aug)
    
    for (k in 1:(K + 1)) {
      if (n_k_mc[k] <= 0) next
      
      log_w_k_temp = log(n_k_mc[k])
      
      prob_z0_mc = dbinom(z0, 1, etaPars_mc[, k], log = TRUE)
    ...
    lambda_Y_z0x_mc[, K_aug] = log(alpha) + logf0_xz0_mc
    lambda_Y_z1x_mc[, K_aug] = log(alpha) + logf0_xz1_mc
...
SURVIVAL_DPMM_POST_ftn = function(object, M, t_grid, ftn = c("S", "f"),
  ...
  if (calc_S) {
    S_z0_store = matrix(NA_real_, nrow = n_MCMC, ncol = n_t)
    S_z1_store = matrix(NA_real_, nrow = n_MCMC, ncol = n_t)
  }
  if (calc_f) {
    f_z0_store = matrix(NA_real_, nrow = n_MCMC, ncol = n_t)
    f_z1_store = matrix(NA_real_, nrow = n_MCMC, ncol = n_t)
  }
  ...
  for (post_reps in 1:n_MCMC) {
    ...
    # ---------------------------------------------------------------------------
    # Build augmented parameter objects: existing K + one new cluster
    # ---------------------------------------------------------------------------
    sig2_new = as.numeric(rscainvchisq_cpp(1, a_sig2, b_sig2))
    beta_new = as.vector(rmvn_cpp(1, a_beta, sig2_new * B_beta))
    eta_new  = rbeta(p_Z, a_eta, b_eta)
    ...
    if (!is.null(condX) && !is.null(p_condX)) {
      ...
      for (k in 1:K) {
        ...
        for (i in seq_along(p_condX)) {
          ...
          val = condX[i]
          if (idx <= p_X1) {
            ll = ll + dbinom(val, 1, piPars_mc[idx, k], log = TRUE)
          } else {
            q = idx - p_X1
            ll = ll + dnorm(val, muPars_mc[q, k], sqrt(tau2Pars_mc[q, k]), log = TRUE)
          }
        }
        log_lambda_x_mc[k] = log(n_k[k]) + ll
      }
      ...
      max_ll = max(log_lambda_x_mc, na.rm = TRUE)
      if (!is.finite(max_ll)) {
        stop("All entries of log_lambda_x_mc are -Inf. Check condX/prior predictive.")
      }
    ...
    for (k in 1:(K + 1L)) {
      ...
      if (p_X2 > 0) {
        for (q in 1:p_X2) {
          ...
          if (is.null(p_condX) || !(col_idx %in% p_condX)) {
            X_mc[ind_S_x_mc, col_idx] = rnorm(
              n_k_temp, muPars_mc[q, k], sqrt(tau2Pars_mc[q, k])
            )
          }
        }
      }
    ...
    if (anyNA(X_mc)) stop("X_mc contains NA in SURVIVAL_DPMM_POST_ftn.")
    
    # ---------------------------------------------------------------------------
    # Outcome mixture over K+1 support
    # ---------------------------------------------------------------------------
    K_aug = K + 1L
    lambda_Y_z0x_mc = matrix(-Inf, nrow = M, ncol = K_aug)
    lambda_Y_z1x_mc = matrix(-Inf, nrow = M, ncol = K_aug)
    
    w_dp_full = c(n_k, alpha) / (M + alpha)
    
    for (k in 1:K_aug) {
      log_w_k_temp = log(w_dp_full[k])
      ...
      if (p_X1 > 0) {
        for (q in 1:p_X1) {
          prob_x_mc = prob_x_mc + dbinom(X_mc[, q], 1, piPars_mc[q, k], log = TRUE)
        }
      }
      if (p_X2 > 0) {
        for (q in 1:p_X2) {
          prob_x_mc = prob_x_mc + dnorm(
            X_mc[, p_X1 + q], muPars_mc[q, k], sqrt(tau2Pars_mc[q, k]), log = TRUE
          )
        }
      }
    ...
    MUy_z0x_mc = matX_z0x_mc %*% betaPars_mc
    MUy_z1x_mc = matX_z1x_mc %*% betaPars_mc
    sqrt_sig2_mc = sqrt(sig2Pars_mc)
    
    # ---------------------------------------------------------------------------
    # Survival / density evaluation
    # ---------------------------------------------------------------------------
    for (ii_t in 1:n_t) {
      curr_t = t_grid[ii_t]
      ...
      if (curr_t <= 0) {
        ...
        if (calc_S) {
          log_S_z0 = S_cond_cpp(curr_log_t, lambda_Y_z0x_mc, MUy_z0x_mc, as.vector(sqrt_sig2_mc), TRUE)
          log_S_z1 = S_cond_cpp(curr_log_t, lambda_Y_z1x_mc, MUy_z1x_mc, as.vector(sqrt_sig2_mc), TRUE)
        ...
        if (calc_f) {
          log_f_z0 = f_cond_cpp(curr_log_t, lambda_Y_z0x_mc, MUy_z0x_mc, as.vector(sqrt_sig2_mc), TRUE)
          log_f_z1 = f_cond_cpp(curr_log_t, lambda_Y_z1x_mc, MUy_z1x_mc, as.vector(sqrt_sig2_mc), TRUE)
...
SURVIVAL_EDPMM_POST = function(object, M, nu, rho, 
  ...
  for (post_reps in 1:n_MCMC) {
    ...
    # ---------------------------------------------------------------------------
    # Build augmented parameter objects:
    #   existing clusters + one new y-cluster
    #   and one new x-subcluster within each existing y-cluster
    # ---------------------------------------------------------------------------
    # new y-cluster outcome parameters
    sig2_new = as.numeric(rscainvchisq_cpp(1, a_sig2, b_sig2))
    beta_new = as.vector(rmvn_cpp(1, a_beta, sig2_new * B_beta))
    betaPars_mc = cbind(betaPars, beta_new)
    sig2Pars_mc = c(as.vector(sig2Pars), sig2_new)
    ...
    etaPars_mc  <- array(NA_real_, dim = c(p_Z,  max_Sx + 1L, Ky + 1L))
    piPars_mc   <- array(NA_real_, dim = c(p_X1, max_Sx + 1L, Ky + 1L))
    muPars_mc   <- array(NA_real_, dim = c(p_X2, max_Sx + 1L, Ky + 1L))
    tau2Pars_mc <- array(NA_real_, dim = c(p_X2, max_Sx + 1L, Ky + 1L))
    
    count <- 1L
    for (k in 1:Ky) {
      max_Kx_Sy_temp <- max_Kx_Sy[k]
      for (r in 1:max_Kx_Sy_temp) {
        etaPars_mc[, r, k] <- etaPars[, count]
        if (p_X1 > 0) piPars_mc[, r, k] <- piPars[, count]
        if (p_X2 > 0) {
          muPars_mc[, r, k] <- muPars[, count]
          tau2Pars_mc[, r, k] <- tau2Pars[, count]
        }
        count <- count + 1L
      }
    }
    
    # new x-subcluster within each existing y-cluster
    for (k in 1:Ky) {
      r_new_k <- max_Kx_Sy[k] + 1L
      
      if (p_Z > 0) etaPars_mc[, r_new_k, k] <- rbeta(p_Z, a_eta, b_eta)
      if (p_X1 > 0) piPars_mc[, r_new_k, k] <- rbeta(p_X1, a_pi, b_pi)
      if (p_X2 > 0) {
        tau2_new_x <- as.numeric(rscainvchisq_cpp(p_X2, a_tau2, b_tau2))
        mu_new_x <- rnorm(p_X2, a_mu, sqrt(tau2_new_x / b_mu))
        muPars_mc[, r_new_k, k] <- mu_new_x
        tau2Pars_mc[, r_new_k, k] <- tau2_new_x
      }
    }
    
    # base x-component for new y-cluster
    k_new = Ky + 1
    if (p_Z > 0) etaPars_mc[, 1, k_new] <- rbeta(p_Z, a_eta, b_eta)
    if (p_X1 > 0) piPars_mc[, 1, k_new] <- rbeta(p_X1, a_pi, b_pi)
    if (p_X2 > 0) {
      tau2_new_x <- as.numeric(rscainvchisq_cpp(p_X2, a_tau2, b_tau2))
      mu_new_x <- rnorm(p_X2, a_mu, sqrt(tau2_new_x / b_mu))
      muPars_mc[, 1, k_new] <- mu_new_x
      tau2Pars_mc[, 1, k_new] <- tau2_new_x
    }
    
    # ---------------------------------------------------------------------------
    # Calculate lambda_y_mc and optional conditional x probabilities
    # ---------------------------------------------------------------------------
    if (!is.null(condX) && !is.null(p_condX)) {
      log_lambda_y_mc = rep(-Inf, Ky + 1L)
      log_prob_Sx_list = vector("list", Ky + 1L)
      ...
      for (k in 1:Ky) {
        ...
        log_w_rk_temp = log(w_rk_temp) - log(sum_w + alpha_omega)
        log_lik_r = rep(-Inf, max_Kx_Sy_temp + 1L)
        ...
        # new x-subcluster inside existing y-cluster
        log_lik_r[max_Kx_Sy_temp + 1L] =
          log(alpha_omega) - log(sum_w + alpha_omega) + log_base_condX
      ...
      # new y-cluster
      log_lambda_y_mc[Ky + 1L] = log(alpha_theta) + log_base_condX
      log_prob_Sx_list[[Ky + 1L]] = 0
      
      max_ll_y = max(log_lambda_y_mc, na.rm = TRUE)
      if (!is.finite(max_ll_y)) {
        stop("All entries of log_lambda_y_mc are -Inf. Check condX/prior predictive.")
      }
      lambda_y_mc = exp(log_lambda_y_mc - max_ll_y)
      
    } else {
      lambda_y_mc = c(n_k, alpha_theta)
      log_prob_Sx_list = vector("list", Ky + 1L)
      log_prob_Sx_list[[Ky + 1L]] = 0
    }
    ...
    Ky_mc = max(max(unique(S_y_mc)), Ky)
    
    # ---------------------------------------------------------------------------
    # Draw S_x and X_mc
    # ---------------------------------------------------------------------------
    S_x_mc = numeric(M)
    n_k_mc = numeric(Ky + 1L)
    n_rk_mc = matrix(0, nrow = nrow(n_rk) + 1L, ncol = ncol(n_rk) + 1L)
    max_Kx_Sy_mc = numeric(Ky + 1L)
    ...
    for (k in 1:Ky_mc) {
      ...
      if (k <= Ky) {
        ...
        if (!is.null(condX) && !is.null(p_condX)) {
          ...
          S_x_sub = sample(1:(max_Kx_Sy_temp + 1L), n_k_temp, replace = TRUE, prob = p_Sx)
        } else {
          w_Sx = c(n_rk[k, 1:max_Kx_Sy_temp], alpha_omega)
          S_x_sub = sample(
            1:(max_Kx_Sy_temp + 1L),
            n_k_temp,
            replace = TRUE,
        ...
        max_Kx_Sy_temp = 1L
        S_x_sub = rep(1L, n_k_temp)
      }
      
      S_x_mc[ind_S_y_mc] = S_x_sub
      max_Kx_Sy_mc[k] = max(S_x_sub)
      max_Kx_fill = max_Kx_Sy_mc[k]
      
      for (r in 1:max_Kx_fill) {
        ind_Sy_x_mc = ind_S_y_mc[S_x_sub == r]
        num_Sy_x_mc = length(ind_Sy_x_mc)
        n_rk_mc[k, r] = num_Sy_x_mc
        if (num_Sy_x_mc == 0) next
        
        if (p_X1 > 0) {
          for (q in 1:p_X1) {
            if (is.null(p_condX) || !(q %in% p_condX)) {
              X_mc[ind_Sy_x_mc, q] = rbinom(num_Sy_x_mc, 1, piPars_mc[q, r, k])
            }
          }
        }
    ...
    # Calculate logf0_x0_mc and logf0_x1_mc
    # ---------------------------------------------------------------------------
    # Calculate logf0_x0_mc and logf0_x1_mc for use in calculating causal effect.
    # Average covariate distribution over prior for each X_mc.
    ab_tau <- a_tau2 * b_tau2
    a_tau_new <- (a_tau2 + 1) / 2
    a_tau_half <- a_tau2 / 2
    b_mu_ratio <- b_mu / (b_mu + 1)
    ...
    # Continuous confounders on log scale
    if (p_X2 > 0) {
      x_mc <- X_mc[, (p_X1 + 1):(p_X1 + p_X2), drop = FALSE]
      logf0_x_mc <- logf0_x_mc + rowSums(
        margin_part1_mc - a_tau_new * log(ab_tau + b_mu_ratio * (x_mc - a_mu)^2)
    ...
    # logf0_z0_mc = log(a_pi/(a_pi+b_pi)) = log(0.5) in this paper
    # logf0_z1_mc = log(b_pi/(a_pi+b_pi)) = log(0.5) in this paper
    logf0_z_mc = log(0.5)
    logf0_xz0_mc = logf0_x_mc + logf0_z_mc
    logf0_xz1_mc = logf0_x_mc + logf0_z_mc
    ...
    lambda_Y_z0x_mc = matrix(-Inf, nrow = M, ncol = Ky_aug)
    lambda_Y_z1x_mc = matrix(-Inf, nrow = M, ncol = Ky_aug)
    for (k in 1:(Ky + 1)) {
      if (n_k_mc[k] <= 0) next
      
      # log_w_k_temp = log(n_k_mc[k]) - log(M + alpha_theta)
      log_w_k_temp = log(n_k_mc[k])
      
      max_Kx_Sy_temp_aug = max_Kx_Sy_mc[k]
      if (max_Kx_Sy_temp_aug <= 0) next
      
      w_rk_temp_aug = n_rk_mc[k, 1:max_Kx_Sy_temp_aug]
      
      n_k_temp_aug = n_k_mc[k]
      log_w_rk_temp_aug = log(w_rk_temp_aug) - log(n_k_temp_aug + alpha_omega)
      
      log_comps_z0 = matrix(-Inf, nrow = M, ncol = max_Kx_Sy_temp_aug + 1)
      log_comps_z1 = matrix(-Inf, nrow = M, ncol = max_Kx_Sy_temp_aug + 1)
      for (r in 1:max_Kx_Sy_temp_aug) {
        prob_x_mc = numeric(M)
        prob_z0_mc = dbinom(z0, 1, etaPars_mc[, r, k], log = TRUE)
        prob_z1_mc = dbinom(z1, 1, etaPars_mc[, r, k], log = TRUE)
        if (p_X1 > 0) {
          for (q in 1:p_X1) {
            prob_x_mc = prob_x_mc +
              dbinom(X_mc[, q], 1, piPars_mc[q, r, k], log = TRUE)
          }
        }
        if (p_X2 > 0) {
          for (q in 1:p_X2) {
        ...
        log_comps_z0[, r] = log_w_rk_temp_aug[r] + prob_z0_mc + prob_x_mc
        log_comps_z1[, r] = log_w_rk_temp_aug[r] + prob_z1_mc + prob_x_mc
      }
      log_comps_z0[, max_Kx_Sy_temp_aug + 1L] =
        log(alpha_omega) - log(alpha_omega + n_k_temp_aug) + logf0_xz0_mc
      log_comps_z1[, max_Kx_Sy_temp_aug + 1L] =
        log(alpha_omega) - log(alpha_omega + n_k_temp_aug) + logf0_xz1_mc
    ...
    lambda_Y_z0x_mc[, Ky_aug] = log(alpha_theta) + logf0_xz0_mc
    lambda_Y_z1x_mc[, Ky_aug] = log(alpha_theta) + logf0_xz1_mc
...
SURVIVAL_EDPMM_POST_parallel = function(object, M, nu, rho,
  ...
  worker_fun = function(post_reps) {
    ...
    for (k in 1:(Ky + 1)) {
      ...
      log_comps_z0 = matrix(-Inf, nrow = M, ncol = max_Kx_Sy_temp_aug + 1L)
      log_comps_z1 = matrix(-Inf, nrow = M, ncol = max_Kx_Sy_temp_aug + 1L)
  ...
  for (post_reps in 1:n_MCMC) {
    ...
    for (ii_t in 1:n_t) {
      ...
      if (curr_t <= 0) {
        ...
        if (calc_S) {
          log_S_z0 = S_cond_cpp(curr_log_t, lambda_Y_z0x_mc, MUy_z0x_mc, sig_mc, TRUE)
          log_S_z1 = S_cond_cpp(curr_log_t, lambda_Y_z1x_mc, MUy_z1x_mc, sig_mc, TRUE)
        ...
        if (calc_f) {
          log_f_z0 = f_cond_cpp(curr_log_t, lambda_Y_z0x_mc, MUy_z0x_mc, sig_mc, TRUE)
          log_f_z1 = f_cond_cpp(curr_log_t, lambda_Y_z1x_mc, MUy_z1x_mc, sig_mc, TRUE)
